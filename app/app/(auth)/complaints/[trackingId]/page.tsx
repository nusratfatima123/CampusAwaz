...
export default async function ComplaintDetailPage({
  ...
  // Fetch identity access requests for anonymous/confidential complaints
  let pendingIdentityRequest: { id: string; requested_by_role: string; created_at: string } | null = null;
  if (complaint.privacy_mode === 'anonymous' || complaint.privacy_mode === 'confidential') {
    const admin = createAdminClient();
    const { data: complaintRow } = await admin
      .from('complaints')
      .select('id')
    ...
    if (complaintRow) {
      ...
      const { data: requestRow } = await admin
        ...
        .select('id, requested_by_role, created_at')
        .eq('complaint_id', complaintId)
        .eq('status', 'admin_approved')
        .order('created_at', { ascending: false })
        .limit(1)
        .maybeSingle();
      pendingIdentityRequest = requestRow as { id: string; requested_by_role: string; created_at: string } | null;
    }
  }
      ...
      {pendingIdentityRequest && (
        <div className="mt-6">
          <IdentityPermissionCard
            requestId={pendingIdentityRequest.id}
            requestedByRole={pendingIdentityRequest.requested_by_role}
          />
        </div>
      )}
