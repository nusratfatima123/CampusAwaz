...
import { assignCounselingRequest } from '@/lib/support';
import type { RoleName } from '@/types/database';

/**
 * POST /api/support/counseling/[id]/assign
 *
 * Assigns a counselor to a counseling request.
...
export async function POST(
  ...
  try {
    ...
    const counselorId = body.counselorId ?? user.id;

    const result = await assignCounselingRequest(
      id,
      counselorId,
    ...
    console.error('[support/counseling/assign] failed:', message);
