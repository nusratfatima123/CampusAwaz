...
const PRIVACY_REQUIRED_CATEGORIES = new Set([
  'safety_harassment',
  'academic',
  'financial',
]);

function buildStepOrder(categoryKey: string): StepKey[] {
  if (PRIVACY_REQUIRED_CATEGORIES.has(categoryKey)) {
    return ['describe', 'privacy', 'evidence', 'review'];
  }
  return ['describe', 'evidence', 'review'];
}

function buildStepLabels(categoryKey: string): string[] {
  if (PRIVACY_REQUIRED_CATEGORIES.has(categoryKey)) {
    return ['Describe', 'Privacy', 'Evidence', 'Review'];
  }
  return ['Describe', 'Evidence', 'Review'];
...
export function ComplaintForm({
  ...
  const stepOrder = buildStepOrder(categoryKey);
  const stepLabels = buildStepLabels(categoryKey);
  const needsPrivacy = PRIVACY_REQUIRED_CATEGORIES.has(categoryKey);

  const [step, setStep] = useState<StepKey>('describe');
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [privacyMode, setPrivacyMode] = useState<PrivacyMode>(
    needsPrivacy ? defaultPrivacyMode : 'confidential',
  );
          ...
          <div className="space-y-6">
            ...
            <AiAssistant
              ...
              onApplyRecommendation={() => {
                // The recommendation is advisory — the student reviews it in the
                // AI panel and can adjust their description before continuing.
              }}
              onApplyDraft={(draft) => {
