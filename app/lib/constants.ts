...
// ---------------------------------------------------------------------------
// Sprint 8 — identity access & authority workflow
// ---------------------------------------------------------------------------

/**
 * Maps complaint category keys to the staff roles that are allowed to handle
 * them. Used to filter the assignment dropdown and validate identity requests.
 */
export const CATEGORY_AUTHORITY_MAP: Record<string, string[]> = {
  academic: ['hod'],
  facilities: ['hod'],
  hostel: ['hostel_warden'],
  financial: ['hod'],
  administration: ['hod'],
  safety_harassment: ['proctor', 'female_focal_person'],
  mental_health: ['counselor'],
  other: ['hod', 'proctor', 'female_focal_person', 'hostel_warden', 'counselor'],
};

/** Human-readable authority labels per category. */
export const CATEGORY_AUTHORITY_LABELS: Record<string, string> = {
  ...
  safety_harassment: 'Proctor / Female Focal Person',
  mental_health: 'Counselor',
  other: 'Administration',
};

/**
 * Fraction of SLA response_hours remaining at which the indicator switches
 * from "on_track" to "approaching". 0.25 = 25% remaining.
 */
export const SLA_APPROACHING_THRESHOLD = 0.25;

/** How long a granted identity access remains valid before auto-expiring. */
export const IDENTITY_ACCESS_GRANT_HOURS = 48;

/**
 * Predefined SLA response windows (in hours) for every category × priority
 * combination. Mirrors the rows seeded in 005_escalation_resolution.sql.
 */
export const SLA_REFERENCE_TABLE: Record<string, Record<string, number>> = {
  academic:          { critical: 24, high: 48, medium: 72, low: 168 },
  facilities:        { critical: 24, high: 48, medium: 72, low: 168 },
  hostel:            { critical: 24, high: 48, medium: 72, low: 168 },
  financial:         { critical: 24, high: 48, medium: 72, low: 168 },
  administration:    { critical: 24, high: 48, medium: 72, low: 168 },
  safety_harassment: { critical: 4,  high: 12, medium: 24, low: 48 },
  mental_health:     { critical: 24, high: 48, medium: 72, low: 168 },
  other:             { critical: 24, high: 48, medium: 72, low: 168 },
};

/** Human-readable priority labels for the SLA table. */
