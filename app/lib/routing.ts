...
/**
 * Filters assignable staff to only those whose role is authorized for the
 * given complaint category, per CATEGORY_AUTHORITY_MAP.
 */
export function filterStaffByCategory(
  ...
  return staff.filter((s) => allowedRoles.includes(s.role));
}
...
export async function applyRoutingDecision(
  ...
  const { data: complaint, error } = await admin
    .from('complaints')
    .select(
      'id, tracking_id, student_id, university_id, category_id, priority, status, is_sensitive, department_id, assigned_to, privacy_mode'
    )
    ...
    await createNotification(
      {
        ...
        body: `A complaint (${complaint.tracking_id}) has been assigned to you.`,
